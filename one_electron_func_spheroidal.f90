subroutine construct_1el_basis_spheroidal(nStates)
  !
  ! Make a spheroidal basis of size nStates.
  ! Start by constructing pseudostates of the one-electron Hamiltonian.
  !
  ! If data_in%use_MSC < 0, the basis is purely one-electron pseudostates.
  ! If        "        = 0, we only use the first diagonalisation for ionisation energy.
  ! If        "        > 0, the states will replace corresponding Laguerre functions.
  ! If data_in%use_sub > 0, replace some Laguerre functions with separate sub-basis.
  !
  ! JS
  !
  use input_data
  ...
  do m = data_in%Mt_min, data_in%Mt_max   ! As specified by the input file.
     do par = 1,-1,-1   ! 1,-1 if parity is a good quantum number, 0 if not.
        ...
        if(nst > 0) then   ! States will be constructed.
           ...
           allocate( overlapMatrix(jFunc,jFunc), HMatrix(jFunc,jFunc), CIMatrix(jFunc,jFunc), energies(jFunc) )
  ...
  call calc_spectro_factors(TargetStates, basis_1e%labot,basis_1e%latop)
...
subroutine write_states(states, nStates)
  !
  ! Print state expansions to file for debugging purposes.
  !
  ! JS
  !
  use input_data
...
real*8 function Hyl_overlap(sturmp, sturm)
  !
  ! Calculate the spheroidal overlap between two Hyleraas functions.
  ! Done analytically if alpha' = alpha.
  !
  ! JS
  !
  use grid_radial
...
real*8 function Hyl_Ham_1e(sturmp, sturm)
  !
  ! Calculate the one-electron Hamiltonian M.E. between two Hyleraas functions.
  !
  ! JS
  !
  use grid_radial
...
subroutine spheroidal_separate(states, statesOid)
  !
  ! Convert one-electron functions (with a spherical harmonic expansion)
  ! to a radial function & spheroidal harmonic, extracting A in the process.
  ! Normalise the angular function s.t. sum(D^2) = 1.
  !
  ! JS
  !
  use grid_radial
  ...
  do jState = 1, nStates
     ...
     do j = 1, nTerms-1
        ...
        write(*,'(8X,A,I2,8X,A,F12.8,8X,A,F16.12,A,F16.8)') 'l =',nint(l), 'D = ',DVec(j), 'A = ',A, ' +/-', stdev
     end do
...
subroutine make_state_basis(bst,states, jSub)
  !
  ! Convert a diagonalised state basis to a sturmian-style basis.
  !
  ! JS
  !
  use MPI_module
...
subroutine make_Hyl_basis(bst,states, nBst,nStates,nSub)
  !
  ! Make a hybrid basis.
  ! Combine the pseudostates, main, and (if applicable) sub bases.
  !
  ! JS 
  !
  use MPI_module
  ...
  type(state), pointer :: stateNew,stateOld
  type(sturmian_nr), pointer :: sturmi,sturmj
  logical :: hlike, found
...
!     if (get_ang_mom_proj(sturmi) /= m) cycle
     ...
     j = 0; found = .false.
     do while ( j<nStates .and. .not.found )
        j = j + 1
